...
// ─── Middleware ──────────────────────────────────────────────
app.use(helmet());
app.use(cors({ origin: process.env.FRONTEND_URL || "http://127.0.0.1:3001", credentials: true }));
app.use(morgan("dev"));
...
const authenticate = (req, res, next) => {
  ...
  // Allow React demo officer login token
  if (token === "demo-officer-token") {
    req.user = {
      nationalId: "OFFICER001",
      name: "Officer Mokoena",
      role: "OFFICER",
      citizenId: null
    };
    return next();
...
// Citizen's own vehicles endpoint used by the frontend
  ...
  try {
    const result = await db.query(
      `SELECT DISTINCT
       ...
       FROM vehicles v
       JOIN ownership o ON o.vehicle_id = v.vehicle_id
       JOIN citizens c ON c.citizen_id = o.owner_id
       LEFT JOIN registrations r ON r.vehicle_id = v.vehicle_id
    ...
    console.error("My vehicles error:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// Draft registration routes MUST come before /api/registrations/:id
app.get("/api/registrations/draft", authenticate, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT r.registration_id, r.current_step, r.draft_data, r.created_at, r.updated_at
       FROM registrations r
       JOIN citizens c ON c.citizen_id = r.owner_id
       WHERE c.national_id = $1
         AND r.is_draft = true
...
app.post("/api/registrations/draft", authenticate, async (req, res) => {
  ...
  try {
    if (draftId) {
      const result = await db.query(
        `UPDATE registrations
         SET draft_data = $1::jsonb, current_step = $2, updated_at = NOW()
         WHERE registration_id = $3
           AND owner_id = (SELECT citizen_id FROM citizens WHERE national_id = $4)
           AND is_draft = true
         RETURNING registration_id, current_step`,
        [JSON.stringify(formData || {}), step || 1, draftId, req.user.nationalId]
      );
      ...
      return res.json({ success: true, draftId: result.rows[0].registration_id, step: result.rows[0].current_step });
    }

    const result = await db.query(
      `INSERT INTO registrations (owner_id, is_draft, draft_data, current_step, status)
       SELECT citizen_id, true, $1::jsonb, $2, 'DRAFT'
       FROM citizens
       WHERE national_id = $3
       RETURNING registration_id`,
      [JSON.stringify(formData || {}), step || 1, req.user.nationalId]
    );
    ...
    res.json({ success: true, draftId: result.rows[0].registration_id, step: step || 1 });
  } catch (err) {
    ...
    await db.query(
      `DELETE FROM registrations
       WHERE registration_id = $1
         AND owner_id = (SELECT citizen_id FROM citizens WHERE national_id = $2)
         AND is_draft = true`,
      [req.params.id, req.user.nationalId]
    );
  ...
  try {
    ...
    await db.query(
      `INSERT INTO ownership (vehicle_id, owner_id, start_date, status)
       ...
       FROM registrations r WHERE r.registration_id = $1
  ...
  try {
    const result = await db.query(
      `SELECT DISTINCT v.*, r.registration_number, r.status as reg_status, r.registration_id,
       ...
       JOIN citizens c ON c.citizen_id = r.owner_id
       LEFT JOIN ownership o ON o.vehicle_id = v.vehicle_id AND o.owner_id = c.citizen_id AND o.status = 'APPROVED'
       WHERE c.national_id = $1
...
app.post("/api/transfers", authenticate, async (req, res) => {
  ...
  if (nationalId === newOwnerNationalId) return res.status(400).json({ success:false, error:"Cannot transfer to yourself." });
  try {
    ...
    const ownerCheck = await db.query(
      "SELECT o.ownership_id FROM ownership o JOIN citizens c ON c.citizen_id=o.owner_id WHERE o.vehicle_id=$1 AND c.national_id=$2 AND o.status='APPROVED'",
    ...
    if (pendingCheck.rows.length > 0) return res.status(400).json({ success:false, error:"A transfer for this vehicle is already pending." });
    const result = await db.query(
    ...
    await db.query("INSERT INTO audit_logs (action,actor_id,actor_role,entity_type,entity_id,details,ip_address) VALUES ($1,$2,$3,$4,$5,$6,$7)",
      ["TRANSFER_REQUESTED", nationalId, "CITIZEN", "ownership", result.rows[0].ownership_id, JSON.stringify({vehicleId,to:newOwnerNationalId}), req.ip]);
    res.json({ success:true, transferId:result.rows[0].ownership_id, newOwnerName:newOwner.full_name, status:"PENDING_APPROVAL", message:"Transfer submitted. Awaiting officer approval." });
  } catch(err) { res.status(500).json({ success:false, error:err.message }); }
});
...
app.patch("/api/transfers/:id/approve", authenticate, async (req, res) => {
  ...
  try {
    ...
    await db.query("UPDATE ownership SET end_date=NOW(), status='CLOSED' WHERE vehicle_id=$1 AND status='APPROVED'", [vehicle_id]);
...
app.patch("/api/transfers/:id/reject", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) return res.status(404).json({ success:false, error:"Transfer not found." });
    await db.query("INSERT INTO audit_logs (action,actor_id,actor_role,entity_type,entity_id,details,ip_address) VALUES ($1,$2,$3,$4,$5,$6,$7)",
...
function calculatePermitFee(type) {
  const fees = {
    A: 150,
    B: 200,
    C: 250,
...
app.get("/api/permits", authenticate, async (req, res) => {
  try {
    ...
    const query = isOfficer
      ? `SELECT p.*, v.vin, v.manufacturer, v.model, v.vehicle_type, c.full_name AS owner_name, c.national_id
         FROM permits p
         JOIN vehicles v ON v.vehicle_id = p.vehicle_id
         JOIN citizens c ON c.citizen_id = p.owner_id
         ORDER BY p.created_at DESC`
      : `SELECT p.*, v.vin, v.manufacturer, v.model, v.vehicle_type
         FROM permits p
         JOIN vehicles v ON v.vehicle_id = p.vehicle_id
         JOIN citizens c ON c.citizen_id = p.owner_id
         WHERE c.national_id = $1
    ...
    console.error("Get permits error:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.post("/api/permits", authenticate, async (req, res) => {
  ...
  const type = String(permitType || "").toUpperCase();
  ...
  if (!["A", "B", "C", "D", "E", "F"].includes(type)) {
    return res.status(400).json({ success: false, error: "Invalid permit type" });
  }
  ...
  try {
    const owner = await db.query(
      `SELECT c.citizen_id
       FROM citizens c
       JOIN ownership o ON o.owner_id = c.citizen_id
       WHERE c.national_id = $1
         AND o.vehicle_id = $2
         AND (o.end_date IS NULL OR o.end_date > NOW())
       LIMIT 1`,
      [req.user.nationalId, vehicleId]
    );

    if (owner.rows.length === 0) {
      return res.status(403).json({ success: false, error: "This vehicle does not belong to the logged-in citizen" });
    }

    const ownerId = owner.rows[0].citizen_id;
    const fee = calculatePermitFee(type);

    const result = await db.query(
      `INSERT INTO permits (vehicle_id, owner_id, permit_type, fee_amount, status)
       VALUES ($1, $2, $3, $4, 'PENDING_APPROVAL')
       RETURNING *`,
      [vehicleId, ownerId, type, fee]
    );
    ...
    if (typeof audit === "function") {
      await audit(
        ...
        "CITIZEN",
        "permit",
        result.rows[0].permit_id,
        { vehicleId, permitType: type, fee },
        req.ip
      );
    }
    ...
    res.json({
      ...
      message: `Permit Type ${type} application submitted successfully. Awaiting officer approval.`,
    });
  } catch (err) {
...
// ══════════════════════════════════════════════════════════════
// OFFICER PERMIT APPROVAL ROUTES
// ══════════════════════════════════════════════════════════════

app.get("/api/permits/pending", authenticate, async (req, res) => {
  if (req.user.role !== "OFFICER") {
    return res.status(403).json({ success: false, error: "Officers only" });
  }
  ...
  try {
    const result = await db.query(
      `SELECT p.*,
              ...
              v.vehicle_type,
              c.full_name AS owner_name,
              c.national_id AS owner_national_id
       FROM permits p
       JOIN vehicles v ON v.vehicle_id = p.vehicle_id
       JOIN citizens c ON c.citizen_id = p.owner_id
    ...
    console.error("Pending permits error:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.patch("/api/permits/:id/approve", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, error: "Permit not found or already processed." });
    }
    ...
    res.json({ success: true, message: "Permit approved successfully.", permit: result.rows[0] });
  } catch (err) {
    console.error("Approve permit error:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.patch("/api/permits/:id/reject", authenticate, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: "Permit rejected.", permit: result.rows[0] });
  } catch (err) {
    console.error("Reject permit error:", err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`\n🚗 DVRS Backend running on http://127.0.0.1:${PORT}`);
  console.log(`📋 Health:       http://127.0.0.1:${PORT}/api/health`);
  console.log(`🔐 MOSIP login:  http://127.0.0.1:${PORT}/api/auth/mosip/login`);
  console.log(`🔍 Registrations:http://127.0.0.1:${PORT}/api/registrations\n`);
});
